...
char* seperate(const char** curr, char d);
...
const char* types[] = {".css", "text/css", ".html", "text/html", 
                    ".gif", "image/gif", ".ico", "image/x-icon", 
                    ".jpg", "image/jpeg", ".js", "text/javascript", 
                    ".php", "text/x-php", ".png", "image/png"};
...
char* indexes(const char* path)
{
    char* temp_file = malloc(strlen(path) + strlen("index.php") + 1);
    temp_file = strcpy(temp_file, path);
    temp_file = strcat(temp_file, "index.php");
    ...
    temp_file = malloc(strlen(path) + strlen("index.html") + 1);
    temp_file = strcpy(temp_file, path);
    temp_file = strcat(temp_file, "index.html");
    if (access(temp_file, F_OK) != -1)
    {
        return temp_file;
    } else
...
bool load(FILE* file, BYTE** content, size_t* length)
{
    ...
    BYTE* buffer = (BYTE*) malloc(capacity * sizeof(BYTE));
    while (fread(buffer + length_temp, sizeof(BYTE), 1, file) == 1)
    {
        length_temp++;
        if (length_temp == capacity)
        {
            ...
            BYTE* temp = (BYTE*) realloc(buffer, capacity * sizeof(BYTE));
            if (temp == NULL)
            {
                free(buffer);
                return false;
...
const char* lookup(const char* path)
{
    char* curr = strrchr(path, '.');
    int n = 16;
    for (int i = 0; i < n; i += 2)
    {
        if (strcasecmp(curr, types[i]) == 0)
        {
            return types[i+1];
        }
    }
...
{
    // Confirm line ends with CRLF
    if (strstr(line, "\r\n") == NULL)
    {
        error(400);
        return false;
    }
    // flag for errors during parsing
    bool err = false;
    // keep track of location in given line
    const char* curr = line;
    // parse for method, request_target, and HTTP_version
    char* method = seperate(&curr, ' ');
    char* request_target = seperate(&curr, ' ');
    char* HTTP_version = seperate(&curr, '\r');
    // if any requests failed, or HTTP_version has a space (ie there were more than 2 spaces),
    // or if request_target has a quote character, throw error 400 and set flag to true
    if (method == NULL || request_target == NULL || HTTP_version == NULL || 
        strchr(HTTP_version, ' ') != NULL || strchr(request_target, '"') != NULL)
    {
        error(400);
        err = true;
    } 
    // if method is not GET, then throw error 405 and set flag to true
    else if (strncmp(method, "GET\0", 4) != 0)
    {
        error(405);
        err = true;
    // if request_target doesn't start with / throw error 501 and set flag to true
    } else if (request_target[0] != '/')
    {
        error(501);
        err = true;       
    // if HTTP version is not correct throw error 505 and set flag to true
    ...
    // keep track of location in request target
    curr = request_target;
    // if it contains ?, then there is a query
    if (strchr(request_target, '?') != NULL)
    {
        char* abs_path_copy = seperate(&curr, '?');
        strcpy(abs_path, abs_path_copy);
        free(abs_path_copy);
        char* query_copy = seperate(&curr, '\0');
        strcpy(query, query_copy);
        free(query_copy);
    } else
    {
        char* abs_path_copy = seperate(&curr, '\0');
        strcpy(abs_path, abs_path_copy);
        free(abs_path_copy);
        query[0] = '\0';
    }
...
char* seperate(const char** curr, char d)
{
    char* left = strchr(*curr, d);
    if (left != NULL)
    {
        int n = left - *curr;
        char* right = (char*) malloc(n + 1);
        strncpy(right, *curr, n);
        right[n] = '\0';
        *curr = left + 1;
        return right;
    } else
